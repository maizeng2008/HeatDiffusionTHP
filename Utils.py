...
from transformerhk_v2.Models import get_non_pad_mask
...
def compute_integral_unbiased(model, data, time, non_pad_mask, type_mask, opt):
    """ Log-likelihood of non-events, using Monte Carlo integration. """

    num_samples = opt.n_samples
    # diff_time: B x L
    diff_time = (time[:, 1:] - time[:, :-1]) * non_pad_mask[:, 1:]
    diff_time_flat = torch.flatten(diff_time)
    temp_time = diff_time.unsqueeze(2) * \
                torch.rand([*diff_time.size(), num_samples], device=data.device)
    # torch.rand: B x L x S
...
def log_likelihood(model, data, time, types, opt):
    ...
    # avg_type_lambda = torch.mean(type_lambda, 0)
    # avg_all_lambda = torch.mean(all_lambda, 0)
    ...
    if opt.integral_solver == 1:
        non_event_ll = compute_integral_unbiased(model, data, time, non_pad_mask, type_mask, opt)
    elif opt.integral_solver == 2:
        non_event_ll = compute_integral_biased(type_lambda, time, non_pad_mask)

    ### Use NN to approx the MCMC
    # non_event_ll = model.linear()
...
def time_loss(prediction, event_time):
    ...
    prediction = prediction[:, :-1]
    prediction = torch.masked_fill(prediction, true < 0, 0)
    true = torch.masked_fill(true, true < 0, 0)


    # event time gap prediction
    loss_fn = torch.nn.MSELoss(reduction="sum")

    se = loss_fn(prediction, true)
